...
<!--                <el-button type="success" size="small" @click="openAdd">添加用户</el-button>-->
            </div>
        </div>

        <div class="mt20" style="padding:0 10px">
            ...
            >
                ...
                <el-table-column prop="adminRole" label="权限" width="180">
                    <template #default="scope">
                        {{scope.row.adminRole===20?'A':'B'}}
                    </template>
                </el-table-column>
                <el-table-column prop="name" label="操作" width="199">
                    <template #default="scope">
                        <span class="blueSpan" @click="openLook(scope.row)">查看</span>
                        <span class="blueSpan ml10" @click="openEdit(scope.row)">编辑</span>
        ...
        <el-dialog
                ...
                :before-close="addHandleClose"   customClass="customWidth">
            <div>
                <user-form v-if="my.editVisible" :editFlag="true" :form-data="my.editInfo"></user-form>
                <div class="mt20 tac">
    ...
    export default {
        ...
        setup() {
            ...
            const saveInfo=async ()=>{
                let param={
                    _id:my.editInfo._id,
                    adminRole:my.editInfo.adminRole,
                };
                await userStore.saveInfo(param).then(res=>{
                    proxy.$message.success("修改权限成功！");
                }).catch(res=>{
                    proxy.$message.error(res.msg);
                });
                await getList();
